...
describe("the splice back to real audio", () => {
	...
	it("splices both channels at the same point", () => {
		// A different phase per channel, so a per-channel alignment would smear the image.
		const source = tone(RATE, 0.2, 997, 0.5, 2);
		const concealed = frames(RATE, 30);
		const expanded = [source[0].subarray(12, 12 + concealed), source[1].subarray(12, 12 + concealed)];
		const back = frames(RATE, BLOCK);
		const returning = [source[0].subarray(0, back), source[1].subarray(0, back)];
		...
		const left = out[0].subarray(0, kept + back);
		const right = out[1].subarray(0, kept + back);
