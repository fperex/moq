...
export function statsTab(parent: Effect, watch: MoqWatch): HTMLElement {
	...
	parent.interval(() => {
		...
		const vConf = watch.video.source.out.config.peek();
		const vCat = watch.video.source.out.catalog.peek();
		const vStats = watch.video.out.stats.peek();
		...
		const aStats = watch.audio.out.stats.peek();
