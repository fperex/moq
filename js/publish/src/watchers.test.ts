...
test("several watchers joining and leaving audio and video keep both tracks live", async () => {
	...
	const announcements: Array<{ pattern: string; active: boolean }> = [];
	const pump = (async () => {
		for (;;) {
			const event = await announced.next();
			if (!event) return;
			announcements.push({ pattern: event.pattern.text, active: event.active });
		}
	})();
	...
	try {
		...
		expect(announcements).toEqual([{ pattern: "call.hang/**", active: true }]);
	} finally {
