...
import { Expired, Producer as GroupProducer, Lagged, MAX_GROUP_FRAMES } from "./group.ts";
...
// Error for either would read as a crash to a caller that only handles StreamError.
test("a budget verdict on unread content is Expired, an eviction is Lagged", async () => {
	const producer = new TrackProducer("test").accept({ maxAge: 5000 });
	const track = producer.subscribe();
	producer.writeString("old");
	...
	const clock = mockMonotonicTime(10_000);
	try {
		const second = new TrackProducer("test").accept({ maxAge: 100 });
		const reader = second.subscribe({ maxAge: 100 });
		...
		expect(lagged).toBeInstanceOf(Lagged);
		expect(lagged).not.toBeInstanceOf(Expired);
		expect((lagged as Lagged).code).toBe(StreamCode.TooFarBehind);
	} finally {
		clock.restore();
	}
});
