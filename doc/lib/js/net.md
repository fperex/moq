...
- **Subscriptions** carry a priority, a `Time.Milli` max age, and optional `groups` bounds. Groups arrive out of order and are read frame by frame, with `Error.TooFarBehind` when a reader asks for a frame the group never held, `Error.Expired` when the max age gives up on a group that still held content, and `Error.GroupTooLarge` when a write exceeds the cache budget and aborts the group.
