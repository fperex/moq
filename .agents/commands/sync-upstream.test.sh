...
if bash "$command_path"; then echo 'Accepted dirty worktree' >&2; exit 1; fi
...
if bash "$command_path"; then echo 'Accepted pending merge' >&2; exit 1; fi
...
cd "$fixture/work"
if bash "$command_path"; then echo 'Accepted conflicting merge' >&2; exit 1; fi
[[ -n $(git ls-files -u) ]]
git merge --abort
[[ $(git rev-parse HEAD) == "$before" && $(cat shared) == fork ]]
[[ -z $(git status --porcelain) ]]
git switch -q --detach
if bash "$command_path"; then echo 'Accepted detached HEAD' >&2; exit 1; fi
