...
- **Subscriptions** carry a priority and max age; groups arrive out of order and are read frame by frame, with `Lagged` when a reader asks for a frame the group never held, `Expired` when the max age gives up on a group that still held content, and `GroupTooLarge` when a write exceeds the cache budget and aborts the group.
